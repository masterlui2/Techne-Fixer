<template>
  <div class="service-form-page">
    ...
    <div class="page-header">
      ...
      <h1>Edit Service</h1>
      <p class="subtitle">Update the details for this service</p>
    </div>
...
const addScopeItem    = (typeId) => form.value.selectedTypes[typeId].push('');
const removeScopeItem = (typeId, idx) => form.value.selectedTypes[typeId].splice(idx, 1);
...
const fetchService = async () => {
  ...
  try {
    ...
    form.value = {
      ...
      // Populate selectedTypes from existing serviceTypes + scopeOfWorks
      selectedTypes: Object.fromEntries(
        (s.serviceTypes ?? []).map(type => [
          type.id,
          (s.scopeOfWorks ?? [])
            .filter(w => w.service_type_id === type.id)
    ...
    if (s.image) {
      existingImage.value = s.image;
      imagePreview.value  = `${baseURL}/storage/${s.image}`;
    } 
  } catch (err) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await axios.post(`/admin/services/${route.params.id}`, fd, {
...
onMounted(async () => {
  await Promise.all([fetchCategories(), fetchServiceTypes(), fetchService()]);
});
</script>
